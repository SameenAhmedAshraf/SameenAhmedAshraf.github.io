...
async function main() {
  ...
  if (!result || result.hit === 0) {
    const btns = (result && result.buttons && result.buttons.length)
      ? "\n\nButtons on page: " + result.buttons.join(' | ')
      : "";
    const flds = (result && result.fields && result.fields.length)
      ? "\n\nInputs on page: " + result.fields.join(', ')
      : "";
    const a = new Alert();
    a.title = "ParkFill — Not Filled";
    a.message = "Could not fill the form.\n\nURL: " + (result ? result.url : d.url) + btns + flds;
    a.addAction("Open anyway");
    a.addCancelAction("Cancel");
    const choice = await a.present();
    if (choice === 0) await wv.present(false);
  } else {
    // Show debug info about what buttons were found, so we can tune if needed
    if (result.buttons && result.buttons.length) {
      console.log("Buttons found at end: " + result.buttons.join(' | '));
    }
...
function fillScript(d) {
  ...
  var d = ${safe};
  var totalHit = 0;
  var completed = false;  // guard — completion() called only once

  // ── Input fill (React-compatible) ──────────────────────────────────────────
  function nv(el, v) {
  ...
  // ── Button helpers ──────────────────────────────────────────────────────────
  // All clickable elements including <a> styled as buttons
  function allBtns() {
  ...
  // Last visible non-back/cancel button — reliable fallback for forward action
  function primaryBtn() {
    var candidates = allBtns().filter(function(el) {
      var txt = (el.textContent || el.value || '').toLowerCase();
      return !/back|cancel|close|skip|previous|prev/i.test(txt);
    });
    return candidates[candidates.length - 1] || null;
  }

  // Explicit form submit button
  function formSubmitBtn() {
    var forms = Array.from(document.querySelectorAll('form'));
    for (var i = 0; i < forms.length; i++) {
      var sub = forms[i].querySelector('button[type="submit"], input[type="submit"]');
      if (sub && sub.offsetParent !== null) return sub;
      var btns = Array.from(forms[i].querySelectorAll('button')).filter(function(b){ return b.offsetParent !== null; });
  ...
  function formVisible() {
    return Array.from(document.querySelectorAll('label')).some(function(l){
      ...
      'input[placeholder*="plate" i],input[placeholder*="make" i],input[placeholder*="apt" i]'
    );
  }
  ...
  function fillAll() {
    ...
    if (d.plate) {
      ...
      q(['input[name="confirm_plate"]','input[name="confirm_license_plate"]',
         'input[name="confirmPlate"]','input[name="plate_confirm"]',
         'input[id*="confirm" i][id*="plate" i]','input[placeholder*="confirm" i]'], d.plate);
    }
    ...
    if (d.email) hit += q([
      'input[type="email"]','input[name="email"]','input[name="guest_email"]',
  ...
  // ── Completion (called only once) ──────────────────────────────────────────
  function done(hit) {
    if (completed) return;
    completed = true;
    var btns = allBtns().map(function(el){
      return (el.textContent || el.value || '').replace(/\\s+/g,' ').trim().slice(0,30);
    }).filter(Boolean).slice(0,10);
    var fields = Array.from(document.querySelectorAll('input,select,textarea')).map(function(el){
      return (el.name || el.id || el.getAttribute('aria-label') || el.placeholder || '').slice(0,25);
    }).filter(Boolean).slice(0,12);
    completion({hit: hit, url: location.href, buttons: btns, fields: fields});
  }
  ...
  function phase2() {
    if (formVisible()) { phase3(); return; }
    var btn = findBtn(/next/i) || findBtn(/continue/i) || primaryBtn();
    if (btn) { btn.click(); setTimeout(phase3, 600); return; }
    if (--p2 > 0) setTimeout(phase2, 300); else phase3();
  }

  // Phase 3 — fill vehicle info (pause so React finishes mounting)
  var p3 = 15;
  ...
  function phase4() {
    ...
    var btn = findBtn(/next/i) || findBtn(/continue/i) || formSubmitBtn() || primaryBtn();
  ...
  function phase6() {
    var btn = findBtn(/send/i) || findBtn(/submit/i) || findBtn(/register/i)
           ...
           || findBtn(/done/i) || formSubmitBtn() || primaryBtn();
